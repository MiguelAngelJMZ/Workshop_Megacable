...
sql_command = f"""CREATE OR REPLACE TABLE workshop_megacable.{usuario}_silver.clientes (
  id_cliente STRING COMMENT 'ID único que identifica a cada CLIENTE',
  genero STRING COMMENT 'Género del cliente: masculino o femenino',
  edad_cliente INT COMMENT 'Edad del cliente',
  casado BOOLEAN COMMENT 'Indica si el cliente está casado: TRUE, FALSE',
  ...
  oferta STRING COMMENT 'Indica la última oferta de marketing que cliente aceptó',
  servicio_telefono BOOLEAN COMMENT 'Indica si el cliente se suscribió al servicio de teléfono',
  prom_mensual_larga_distancia DOUBLE COMMENT 'Indica los cargos del cliente por llamada internacionales',
  multiples_lineas BOOLEAN COMMENT 'Indica si el cliente tiene más de una línea teléfonica',
  servicio_internet BOOLEAN COMMENT 'Indica si el cliente se suscribió al servicio de internet',
  tipo_internet STRING COMMENT 'Indica el tipo de servicio de internet que el cliente ha contratado',
  prom_mensual_gb_descarga INT COMMENT 'Indica el promedio de GB de descarga mensual que el cliente ha consumido',
  seguridad_online BOOLEAN COMMENT 'Indicia si el cliente se suscribió al un plan adicional de seguridad',
  backup_online BOOLEAN COMMENT 'Indica si el cliente se suscribió al un plan adicional de backup',
  plan_proteccion_dispositivos BOOLEAN COMMENT 'Indica si el cliente se suscribió al un plan adicional de protección para su equipo de internet',
  soporte_premium BOOLEAN COMMENT 'Indica si el cliente se suscribió al un plan adicional de soporte premium con tiempos reducidos de atención',
  tv_streaming BOOLEAN,
  peliculas_streaming BOOLEAN COMMENT 'Indica si el cliente se suscribió al servicio de películas en streaming con un tercero sin costo adicional',
  musica_streaming BOOLEAN,
  datos_ilimitados BOOLEAN COMMENT 'Indica si el cliente se suscribió al servicio de datos ilimitados para descarga y subida',
  tipo_contrato STRING,
  recibo_electronico BOOLEAN COMMENT 'Indica si el cliente se suscribió al servicio de recibir el recibo de su factura por correo electrónico',
  tipo_pago STRING,
